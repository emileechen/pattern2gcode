...
**Step 3 — Pieces.** This is the human-review step, and it matters: detection is reliable but not infallible, and only you know which shapes are real pattern pieces. Each detected piece appears as a red dashed overlay on the canvas and as a card in the list. Tap a piece on the canvas, or its card, to **select** it: the canvas scrolls to it if needed, it gets a frame and a solid blue outline, and an edit bar appears above the canvas with everything you can do to it — **Drop/Keep** (dropped pieces turn gray and are left out of export; the Delete key does the same), the name (tap to rename), **Set grain**, **Zoom to** (fills the view with the piece), **Reset outline**, and **Done**. Tap empty canvas or press Esc to deselect. Names become the export file names, so `p3-piece-7` renamed to `bodice-front` exports as part of a sheet listing `bodice-front`. The header shows a running count of kept versus found.
