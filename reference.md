...
pattern2gcode turns a scanned (non-vector) sewing-pattern PDF into G-code that a 3D printer traces directly onto fabric, printing each piece's cutting outline as a thin plastic line. It runs entirely in your browser from a single file, `pattern2gcode.html`. Your PDF is never uploaded anywhere; the only network access is loading the PDF-reading library from a CDN when the page opens.
